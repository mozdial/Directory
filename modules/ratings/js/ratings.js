...
function rateStars(spn) {
  ...
  if(spn.parentNode.parentNode) {
    ...
    if (navigator.userAgent.indexOf("Firefox")!=-1) star_sp.setAttribute('style', 'width:'+((i_sp+1)*22)+'px; background:url("modules/ratings/images/star2.png") repeat-x top left;');
    else {
...
function addRtgData(elm, totalrate, nrrates, renot) {
  ...
  var htmlrtg = '';
	  ...
	  htmlrtg += d_rtg+'('+avgrating+') '+nrrates+' '+RatingsVoc._MSG["votes"]+'<div id="thankyou"></div>';
	  htmlrtg += '</div>';
...
function thankyou(action){
  document.getElementById('thankyou').innerHTML = (action != null && action == 'clear') ? '' : '<i><b>'+RatingsVoc._MSG["thanks_for_rating"]+'</b></i>';
}
