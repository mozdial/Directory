...
if($objLogin->IsLoggedInAs('owner','mainadmin') && Modules::IsModuleInstalled('listings')){	
	...
	if(!empty($listing_id) && !empty($listing_info_name)){
		
		$objListingsCategories = new ListingsCategories($listing_id);
		...
		// Start main content
		draw_title_bar(
			prepare_breadcrumbs(array(_LISTINGS_MANAGEMENT=>'',_LISTINGS=>'',$listing_info_name=>'',_CATEGORIES=>'',ucfirst($action)=>'')),
			prepare_permanent_link('index.php?admin=mod_listings_management', _BUTTON_BACK)
		);
