...
if($objLogin->IsLoggedInAs('owner','mainadmin')){
	...
	if($tabid == '1_1') {
		...
		$tab_content_1 = '<form action="index.php?admin=settings" method="post">
			...
			<tr valign="top">
				<td width="150px">'._OFFLINE_MESSAGE.': <img class="help" src="images/question_mark.png" title="'._SITE_OFFLINE_MESSAGE_ALERT.'" /></td>
				<td colspan="2">
					<textarea class="form_text" name="offline_message" cols="54" rows="3">'.decode_text($params["offline_message"]).'</textarea>
					<br />'._OFFLINE_LOGIN_ALERT.' 
				</td>
			</tr>
			<tr valign="top">
				<td width="150px">'._SEO_URLS.': <img class="help" src="images/question_mark.png" title="'._SEO_LINKS_ALERT.'"></td>
				<td colspan="2">
			...
			<tr valign="top">
				<td width="150px">'._WYSIWYG_EDITOR.':</td>
				<td colspan="2">
			...
			<tr valign="top">
				<td width="150px">'._RSS_FEED_TYPE.': </td>
				<td colspan="2">
			...
			<tr valign="top" id="cache_lifetime_row" style="'.(($params['caching_allowed'] == '1') ? '' : 'display:none').';">
				<td width="150px">'._CACHE_LIFETIME.': </td>
		...
		<legend>'._META_TAGS.'</legend>
			...
			<tr><td colspan="2" nowrap height="5px"></td></tr>
			<tr>
				<td colspan="2" style="padding-left:5px;"><input class="form_button" type="submit" name="btnSubmit" value="'._BUTTON_CHANGE.'"></td>
			</tr>
			</table>
		</form>
		</fieldset>';
		...
		'.draw_token_field(false).'
		    ...
		    <tr valign="top">
				...
				<td colspan="2">
					...
					<option '.(($params_tab3["price_format"] == "european") ? "selected" : "").' value="european">1.234,00 (european)</option>
					<option '.(($params_tab3["price_format"] == "american") ? "selected" : "").' value="american">1,234.00 (american)</option>
					</select>
				</td>
		    </tr>
			<tr><td colspan="3" nowrap height="5px"></td></tr>
		...
		$tab_content_4 = '<form id="frmEmailSettings" action="index.php?admin=settings" method="post">
			...
			<tr valign="top">
				...
				<td style="border-left:1px solid #d1d2d3;padding:2px 18px 2px 18px">
				...
				<td width="148px">
					...
					<option value="">-- '._SELECT.' --</option>';					
						...
						foreach($arr_templates as $key){
							$tab_content_5 .= '<option '.(($template == $key) ? 'selected="selected"' : '').' value="'.$key.'">'.ucfirst($key).'</option>';
						}
						$tab_content_5 .= '
					</select>
				</td>
		...
		$session_support = isset($phpinfo['session']['Session Support']) ? $phpinfo['session']['Session Support'] : _UNKNOWN;
		...
		$tab_content_6 = '<ul '.((Application::Get('lang_dir') == 'rtl') ? 'style="list-style:none"' : '').'>
