...
if($objLogin->IsLoggedInAs('owner','mainadmin')){
	...
	if($tabid == '1_1'){		
		$sql = 'SELECT
					COUNT(*) as cnt,
					cnt.abbrv as country_abbrv,
					cnt.name as country_name
				FROM '.TABLE_CUSTOMERS.' c
					INNER JOIN '.TABLE_COUNTRIES.' cnt ON c.b_country = cnt.abbrv
				GROUP BY cnt.abbrv';
		...
		$second_tab_content = '
			<script type="text/javascript">
				...
				$sql = 'SELECT
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'01\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month1,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'02\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month2,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'03\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month3,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'04\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month4,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'05\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month5,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'06\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month6,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'07\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month7,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'08\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month8,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'09\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month9,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'10\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month10,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'11\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month11,
					(SELECT '.$selStatType.' FROM '.TABLE_CUSTOMERS.' c '.$join_clause.' WHERE SUBSTRING(c.date_created, 6, 2) = \'12\' AND SUBSTRING(c.date_created, 1, 4) = '.(int)$year.' '.$where_clause.') as month12
