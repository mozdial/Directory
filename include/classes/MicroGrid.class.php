 ...
 *
 *  1.7.9
 *      - added possibility to add watermark with: 'watermark'=>false, 'watermark_text'=>''
 *      -
 *      -
 *      -
 *      -
 *  1.7.8
 *      - bug fixed in using BeforeDetailsRecord
 *      - added attribute "custom_handler" for filtering fields
 *      - bug fixed with drawing post_html for "enum" fields in details mode 
 *      - added new parameter - buttons to DrawAddMode()
 *      - removed 2 params $operation & $operation_field from DrawEditMode()
 *  1.7.7 
 ...
 *      - improved drawing alter color procedure
 *  1.7.6
...
class MicroGrid {
	 ...
	 *	
	 ***********************************************************************/
	public function DrawAddMode($buttons = array('cancel'=>true))
	{		
	...
	private function ValidateField($key, $val)
	{
		...
		if(isset($this->params[$key]) && $this->params[$key] != ''){		
			...
			}else if($validation_type == 'alpha_numeric' && !self::IsAlphaNumeric($this->params[$key])){
	...
	{
		$strict = false;
		$regex = $strict ? '/^([.0-9a-z_-]+)@(([0-9a-z-]+\.)+[0-9a-z]{2,7})$/i' :  '/^([*+!.&#$¦\'\\%\/0-9a-z^_`{}=?~:-]+)@(([0-9a-z-]+\.)+[0-9a-z]{2,7})$/i';
    ...
    */
	...
	public static function GetParameter($param, $use_prefix = true, $u_prefix = '')
	{
		...
		if($type == 'numeric' && !self::IsNumeric($output)){
			$output = '0';
		}else if($type == 'alpha_numeric' && !self::IsAlphaNumeric($output)){
			$output = '';
		}
		return $output;
	}
